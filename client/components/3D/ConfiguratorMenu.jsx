    ...
    if (store.initialized) {
        return (
            <div className='absolute max-md:w-full px-4 py-4 flex flex-col  rounded-sm right-0 md:w-1/3 bg-[#00000005] h-[calc(100vh-106px)]'>
                ...
                <div className='w-full my-5 h-5/6 overflow-y-auto scrolable-block_gray pr-[5px] flex flex-col gap-y-3'>
                    <div className='w-full flex flex-col gap-y-2 bg-white py-2 px-2 rounded-t-lg'>
                        <h2>Shape of the table top</h2>
                        <div className='w-full flex flex-row flex-wrap gap-x-[3%] gap-y-2'>
                            ...
                            {store.tabletopShapes.map((shape, index) => (
                                ...
                                >
                                    {shape.imgSrc && <div>{shape.imgSrc}</div>}
                    ...
                    <div className='w-full flex flex-col gap-y-2 bg-white py-2 px-2 rounded-b-lg'>
                        <h2>Type of legs</h2>
                        <div className='w-full flex flex-row flex-wrap gap-x-[3%] gap-y-2'>
                            {store.legsTypes.map((leg, index) => (
                                <div
                                    ...
                                    className={'w-[30%] flex py-1 px-1 flex-col bg-[#00000007] rounded-lg border transition-all hover:bg-[#00000010] cursor-pointer' + `${leg.imgSrc ? " h-20 justify-between items-center" : " h-10 justify-center items-center"}` + `${store.selected?.legsType === leg ? ' border-[#bd8448]' : ' border-transparent'}`}
                                >
                                    {leg.imgSrc && <div>{leg.imgSrc}</div>}
